...
st.markdown('''Generally, voice speed would be much faster than writing speed. 
...
if agree:    
    ...
    with st.spinner('Processing audio...'):
        ...
        labels = cluster_seg(order_list, min_cluster= 2, max_cluster= 100, n_features= 200, n_mfcc= 12)
    st.success('Done!')

    st.header('Transcript')
    with st.spinner('Transcripting...'):
        if len(data) / sample_rate >= 60:
            ...
            work_in_local(order_list, labels, transcript_path= TRANSCRIPT_PATH, sample_rate= sample_rate)
    st.success('Done')
